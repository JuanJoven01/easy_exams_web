...
import {removesExamAPI} from '../services/exams'

const ExamsEliminatorComponent = ({courseId, name, setIsExamEliminator}) => {

    const {setModal, setIsLoading } = useGlobalContext()


    const closeModalHandler = () => {
        setIsExamEliminator(false)
    }

    const handleSubmit = async (e) => {
        e.preventDefault();
        setIsLoading(true)
        const response   =  await removesExamAPI(
            courseId
        )
    ...
    return (
        <section className="flex justify-center absolute z-50 w-full backdrop-grayscale-100  ">
                <div className="mb-10 mx-5 group relative w-96 overflow-hidden rounded-2xl bg-gray-300 p-1 transition-all duration-300 ease-in-out hover:bg-gradient-to-r hover:from-blue-500 hover:via-slate-200 hover:to-charlie">
                    ...
                    <h2 className="text-center text-2xl font-bold text-red-400 mb-6">{`Are you sure that wants to removes the exam named:`}</h2>
...
ExamsEliminatorComponent.propTypes = {
    courseId: PropTypes.number.isRequired,
    name: PropTypes.string.isRequired,
    setIsExamEliminator : PropTypes.func.isRequired,
}

export default ExamsEliminatorComponent;
